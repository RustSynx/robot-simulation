...
JointAngles InverseKinematics::solve_ik_3dof(double x, double y, double z) {
  ...
  joint_angles.theta1 = std::atan2(y, x) + std::asin(0.127/r);
  ...
  double r_planar = std::sqrt(r*r - 0.127*0.127);
  double d = std::sqrt(r_planar*r_planar + r_v*r_v);
  ...
  double inner_angle_2 = std::atan2(z - 0.14, r);
